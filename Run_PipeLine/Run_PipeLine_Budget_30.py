...
def main():
    ...
    iterations = 20
    selection_criteria = [('Hybrid', 'BADGE_30'), ('Uncertainty', 'pred_prob_30')]
    num_epoch = 15
    budget_per_iter = 30
    accuracy_scores_dict = defaultdict(list)

    for approach, criterion in selection_criteria:
        set_seed()
        resnet = ourResNet()
        ...
        AL_class = ActiveLearningPipeline(model=model,
                                          ...
                                          train_df=train_df,
                                          appraoch=approach, C0=0.5)
        accuracy_scores_dict[criterion] = AL_class.run_pipeline()
    with open('Budget_30_accuracy.pkl', 'wb') as file:
